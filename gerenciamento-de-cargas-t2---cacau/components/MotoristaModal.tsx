...
const MotoristaModal: React.FC<MotoristaModalProps> = ({ onClose, onSelect }) => {
  ...
  const handleClearSelection = () => {
    console.log('[MotoristaModal] Limpando seleção');
    onSelect({ 
      MOTORISTA: '', 
      CAVALO: '', 
      CARRETA: '' 
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden max-h-[85vh] flex flex-col">
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
           <button 
             ...
             className="px-6 py-2 bg-red-500 hover:bg-red-600 rounded-xl text-sm font-bold text-white transition-colors shadow-sm"
           >
             Limpar Seleção
           </button>
           <button 
             onClick={onClose} 
             className="px-6 py-2 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-100 transition-colors"
           >
             Fechar
           </button>
        </div>
