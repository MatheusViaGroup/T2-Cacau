...
const Admin: React.FC = () => {
  ...
  const [data, setData] = useState({
    origens: ['Fábrica Ilhéus', 'CD Itabuna', 'Porto Salvador'],
    destinos: ['Fábrica Jundiaí', 'Cliente Nestlé', 'Porto de Santos'],
    contatos: [
      { name: 'João Silva', whatsapp: '5573999999999' },
      { name: 'Maria Oliveira', whatsapp: '5573888888888' }
    ]
  });
  ...
  const handleAdd = () => {
    ...
    if (activeTab === 'contatos') {
      setData(prev => ({ ...prev, contatos: [...prev.contatos, { name: newItem, whatsapp: newMetadata }] }));
    } else {
  ...
  const handleDelete = (index: number) => {
    if (activeTab === 'contatos') {
      setData(prev => ({ ...prev, contatos: prev.contatos.filter((_, i) => i !== index) }));
    } else {
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-8">
          <div className="mb-8 bg-gray-50 p-6 rounded-2xl border border-gray-100">
            <h4 className="text-xs font-bold text-gray-500 uppercase mb-4">Adicionar Novo Registro</h4>
            <div className="flex gap-4">
              <input
                type="text"
                placeholder={activeTab === 'contatos' ? 'Nome do Motorista' : `Nova ${activeTab.slice(0, -1)}`}
          ...
          <div className="space-y-2">
            {activeTab !== 'contatos' ? (
              (data[activeTab] as string[]).map((item, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl hover:border-primary/20 hover:bg-gray-50 transition-all group">
              ...
              data.contatos.map((contact, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl hover:border-primary/20 hover:bg-gray-50 transition-all group">
                  <div>
                    <div className="font-bold text-gray-800">{contact.name}</div>
                    <div className="text-sm text-gray-500 font-mono">{contact.whatsapp}</div>
                  </div>
                  <div className="flex items-center gap-4">
                     <a href={`https://wa.me/${contact.whatsapp}`} target="_blank" rel="noreferrer" className="text-emerald-500 hover:text-emerald-600">
            ...
            {(activeTab === 'contatos' ? data.contatos.length : data[activeTab].length) === 0 && (
