...
export const AdminPage = () => {
  ...
  const refreshData = async () => {
    setIsLoading(true);
    try {
        const [o, d, c, f] = await Promise.all([
  ...
  const handleAddContato = async () => {
    ...
    const exists = contatos.find(c => c.motoristaNome === selectedMotorista);
    let idToUse = '';
    
    if (exists) {
        if(!confirm(`O motorista ${selectedMotorista} já possui um telefone cadastrado (${exists.telefone}). Deseja atualizar?`)) {
            return;
        }
        idToUse = exists.id;
    }
    ...
    const payload: ContatoMotorista = {
        id: idToUse, // Empty or existing ID
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <ul className="divide-y divide-gray-100">
          {activeTab === 'contatos' ? (
              ...
              // Locations List
              <>
                 {(activeTab === 'origens' ? origens : destinos).length === 0 && (
                    <li className="p-8 text-center text-gray-400">
